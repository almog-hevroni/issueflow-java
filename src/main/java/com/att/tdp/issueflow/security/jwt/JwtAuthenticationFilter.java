...
import com.att.tdp.issueflow.security.auth.TokenRevocationService;
...
public class JwtAuthenticationFilter extends OncePerRequestFilter {
	...
	protected void doFilterInternal(
		...
		if (tokenRevocationService.isRevoked(tokenDetails.jti())) {
			filterChain.doFilter(request, response);
			return;
		}

		JwtPrincipal principal = jwtService.extractPrincipal(token);
		AuthUserDetails userDetails = (AuthUserDetails) authUserDetailsService.loadUserByUsername(principal.username());
